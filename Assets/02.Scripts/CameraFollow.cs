...
public class CameraFollow : MonoBehaviour
{
    ...
    public float bottomLimit = -3f; // 아래쪽 카메라 이동 제한 (추가)
    public float topLimit = 3f;     // 위쪽 카메라 이동 제한 (추가)
    ...
    void LateUpdate()
    {
        ...
        // X, Y 위치가 일정 범위 내에서만 이동하도록 제한
        float clampedX = Mathf.Clamp(desiredPosition.x, leftLimit, rightLimit);
        float clampedY = Mathf.Clamp(desiredPosition.y, bottomLimit, topLimit);
    ...
    void RestrictPlayerPosition()
    {
        ...
        // Y축도 제한
        if (player1ViewportPos.y < 0.02f)
            player1.position = mainCamera.ViewportToWorldPoint(new Vector3(player1ViewportPos.x, 0.02f, player1ViewportPos.z));
        else if (player1ViewportPos.y > 0.98f)
            player1.position = mainCamera.ViewportToWorldPoint(new Vector3(player1ViewportPos.x, 0.98f, player1ViewportPos.z));
        ...
        // Y축도 제한
        if (player2ViewportPos.y < 0.02f)
            player2.position = mainCamera.ViewportToWorldPoint(new Vector3(player2ViewportPos.x, 0.02f, player2ViewportPos.z));
        else if (player2ViewportPos.y > 0.98f)
            player2.position = mainCamera.ViewportToWorldPoint(new Vector3(player2ViewportPos.x, 0.98f, player2ViewportPos.z));
    }
}